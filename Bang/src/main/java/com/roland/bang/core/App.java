...
public class App extends ListenerAdapter {
    ...
    public static void main(String[] args) throws LoginException, IllegalArgumentException, InterruptedException, RateLimitedException, FileNotFoundException, ClassNotFoundException, IOException {
        ...
        jdaBot.getPresence().setGame(net.dv8tion.jda.core.entities.Game.of("type ;help"));
    ...
    public void onMessageReceived(MessageReceivedEvent e) {
        ...
        if (e.getAuthor().isBot()) {
            ...
            if (e.getAuthor().getId().equals(game.currentPlayer.getPlayerID())) {
                if (game.currentInput != -1 && !message.contains(" ")) {
                    game.processInput(message);
                    return;
                }
            }
            // Check for ability
            if (game.getAAPlayer() != null && e.getAuthor().getId().equals(game.getAAPlayer().getPlayerID())) {
                game.processAbility(message);
            }
        }
    }
